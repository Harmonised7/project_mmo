...
//		}
		...
		//Total xp 10^x milestone broadcast
		{
			...
			if(nextTotalXpMilestone < getTotalXpFromMap(Config.getXpMap(player)))
				ServerUtil.sendMsgToAll(new TranslationTextComponent("pmmo.milestoneTotalXp", playerName, nextTotalXpMilestone).setStyle(textStyle.get("green")), false);
		}
		...
		//10^x Xp in skill milestone broadcast
		{
			...
			if(nextXpMilestone < Skill.getXp(skill, uuid))
				ServerUtil.sendMsgToAll(new TranslationTextComponent("pmmo.milestoneXpInSkill", playerName, nextXpMilestone, skill).setStyle(Skill.getSkillStyle(skill)), false);
		}
